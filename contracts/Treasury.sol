...
contract BabelTreasury is Ownable {
    ...
    address public sBABEL;
    uint public sBABELQueue; // Delays change to sBABEL address
    ...
    /**
        @notice allow approved address to deposit an asset for BABEL
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BABEL needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable(BABEL).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BABEL for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IBabelERC20(BABEL).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20(sBABEL).balanceOf( msg.sender ); // Can only borrow against sBABEL held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BABEL
        @param _amount uint
     */
    function repayDebtWithBABEL( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IBabelERC20(BABEL).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BABEL, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable(BABEL).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20(BABEL).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BABEL valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BABEL decimals
            value_ = _amount.mul( 10 ** IERC20(BABEL).decimals() ).div( 10 ** IERC20( _token ).decimals() );
